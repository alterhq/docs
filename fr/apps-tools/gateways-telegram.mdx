...
<Steps>
  ...
  <Step title="Choisir Action par défaut">
    **Ask Anything** est la valeur par défaut et fonctionne sur Telegram immédiatement. Pour une action personnalisée, ajoutez `telegram` ou `*` sous **Action Editor → Model → Gateways → Gateways autorisés**.
  </Step>
