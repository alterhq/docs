...
<Steps>
  ...
  <Step title="デフォルト Action とスレッド動作を選択">
    **Ask Anything** がデフォルトで、Discord ですぐ使えます。カスタム Action には **Action Editor → Model → Gateways → 許可された Gateways** に `discord` または `*` を追加します。

    **動作** で、必要に応じて **サーバーでの議論をスレッドで開始** をオンにすると、各新しいサーバートップレベルメッセージに専用 Discord スレッドと Alter チャットが付きます。既存スレッド内のメッセージはそのチャットを継続します。DM は変わりません。使用する場合は bot 招待時（**ステップ 4**）に **Create Public Threads** と **Send Messages in Threads** を付与してください。
  </Step>
  <Step title="有効化して保存">
