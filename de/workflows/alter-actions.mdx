...
**Ask Anything-Standards (beta100+):** Die integrierte **Ask Anything**-Action aktiviert standardmäßig **Flow**, **Computer Use** und **Ask User**. Passen Sie sie an oder duplizieren Sie sie, wenn Sie ein schmaleres Tool-Set möchten.

**Memory:** Verknüpfen Sie das **Memory**-Tool, wenn eine Action gespeicherte Fakten lesen oder frühere Chats und Meetings durchsuchen soll. Memory ist nicht standardmäßig für jede Action aktiviert. Siehe [Memory](/de/workflows/memory) und [Memory verwalten](/de/how-to/manage-memory).

Die integrierte **Memory Catch-Up**-Action bringt Memory bereits verknüpft mit. Das ist Alter's vertrauenswürdiger Weg, Memory aus den letzten 30 Tagen von Chats und Meetings zu befüllen — empfohlen, wenn Sie Memory zum ersten Mal aktivieren oder kürzliche Historie nachfüllen möchten. Siehe [Memory Catch-Up](/de/workflows/memory-catch-up).

**Coding-Tools (post-beta106):** Der schnellste Weg ist ein [Marketplace-Coding-Agent](/de/workflows/coding-tools#get-started-from-the-marketplace-recommended) — laden Sie ihn unter **Tools Manager → Marketplace → Browse** herunter (z. B. den live [ChatGPT Agent](https://alterhq.com/marketplace/chatgpt-agent)) und führen Sie ihn einmal aus; Alter aktiviert die erforderliche Coding-Integration bei der ersten Nutzung. Alternativ verbinden Sie unter **Local Tools → Coding** zuerst **ChatGPT**, **Cursor**, **OpenCode** und/oder **Claude** im **Tools Manager** und schränken Sie dann ein, welche Anbieter jede Action nutzen darf — z. B. könnte eine Release-Action nur **Cursor** aktivieren. Coding-Sitzungen laufen asynchron; Ergebnisse kehren zur ursprünglichen Unterhaltung zurück. Siehe [Coding-Tools](/de/workflows/coding-tools).
...
Wählen Sie das AI-Modell, Thinking effort und das Kreativitätsniveau.

![Modell Tab](/images/tools/action-editor-07-model.webp)

**Felder:**

- **Modell** — Durchsuchbare Auswahl gruppiert in **Current**, **Favorites** und **Other models**. Leer lassen für das Standardmodell des Nutzers oder ein bestimmtes Modell für diese Action anheften.
  - Jede Zeile zeigt Kontextlänge, Vision-Unterstützung und Favoriten
  - Wenn das Modell es unterstützt, **Thinking effort** (**Low**, **Balanced**, **Max**) inline anpassen
...
Alter merkt sich Thinking effort **pro Modell** in Hub, QuickHub und Action Editor.

<Tip>
  Nutzen Sie **⌘F** in der Action-Editor-Seitenleiste zum Suchen von Actions. Klicken Sie auf das Filtersymbol, um nach Hintergrundjobs, Zeitplänen, Warnungen, geänderten Actions oder **Folders** (Action-Kategorien) zu filtern. Ordnerfilter bieten **All**- und **None**-Kurzbefehle.
</Tip>

### 8. Gateways

Wählen Sie, ob diese Aktion über **Discord**- und **Telegram**-Gateways verfügbar ist.

**Verfügbarkeit** — Radiooptionen:

| Einstellung | Wirkung |
|---------|--------|
| **Keine** | Nur lokal (Hub, QuickHub usw.) |
| **Discord** | Verfügbar über Discord-Gateways |
| **Telegram** | Verfügbar über Telegram-Gateways |
| **Alle** | Verfügbar über jedes Gateway |

Die mitgelieferte **Ask Anything**-Aktion ist standardmäßig auf **Alle**. Benutzerdefinierte Aktionen starten auf **Keine**, bis Sie eine Plattform oder **Alle** aktivieren. Setzen Sie Ask Anything auf **Keine**, wenn sie nur lokal sein soll.

Auf Gateways starten **parameterlose Aktionen** (ohne Eingabefelder) sofort über **Choose action**; **parametrisierte Aktionen** warten auf die nächste Nachricht des Nutzers.

**Geplante Gateway-Zustellung** — Läuft eine Action nach Zeitplan, aktivieren Sie **Start scheduled conversations on a gateway**, wählen Sie ein aktiviertes **Discord**-, **Telegram**- oder **Buzz**-Gateway *(intern)* und dann ein **Destination** (Kanal, Chat oder Buzz-Stream). Alter postet die Eröffnungsantwort der Action dort, statt nur lokal zu laufen. Buzz-Gateways können auch ein **Default scheduled destination** unter **Einstellungen → Integrations → Gateways** setzen.

Siehe [Gateways](/apps-tools/gateways) für Einrichtung, Chat-Befehle, geplante Zustellung und Gateway-Einstellungen.
...
- [Tools & Integrationen](/de/apps-tools/integrations-overview) — Verfügbare Tools für Ihre Aktionen
- [URL Callbacks](/de/workflows/url-callbacks) — Detaillierte Untersuchung der x-callback-url-Automatisierung
- [Sprache & Diktat](/de/workflows/dictation) — Sprachtrigger einrichten
- [Einstellungen](/de/how-to/settings-guide) — Globale Modell- und Spracheinstellungen konfigurieren
