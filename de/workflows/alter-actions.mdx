...
- **Erlaubte Gateways** — Kommagetrennte Liste: `discord`, `telegram` oder `*` für alle Gateways. Leer lassen, um die Aktion nur lokal zu halten (Hub, QuickHub usw.).
- Die mitgelieferte **Ask Anything**-Aktion enthält standardmäßig **Discord** und **Telegram**. Benutzerdefinierte Aktionen benötigen den Plattformnamen oder `*` unter **Erlaubte Gateways**.

Siehe [Gateways](/apps-tools/gateways) für Einrichtung und Chat-Befehle.
