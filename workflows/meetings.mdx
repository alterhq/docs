...
<Steps>
  <Step title="Enable auto-record">
    Settings → Voice → **Auto Record Meetings**
  </Step>
...
<Steps>
  <Step title="Start recording">
    Click the mic icon in QuickHub (or the notch when QuickHub Mode is off) — it turns into a red recording button
  </Step>
  <Step title="Monitor">
...
### Speaker name matching (beta93+)

When cloud transcription is available, enable **Find speakers names (experimental)** in **Settings → Voice → Automation**. Alter can suggest real names instead of generic "Speaker 1" labels:

- **Calendars for speaker matching** — Choose which Apple calendars to use. Alter matches the recording to calendar events and attendee lists.
- **Contacts for speaker matching** — Grant Contacts access so attendee emails resolve to display names.
- **Manual calendar event selection** — Link a transcript to a specific calendar event when auto-match is wrong.

<Steps>
  <Step title="Open Voice settings">
    **Settings → Voice → Automation**
  </Step>
  <Step title="Enable speaker matching">
    Turn on **Find speakers names (experimental)**
  </Step>
  <Step title="Select calendars">
    Pick the calendars that hold your work meetings (or select all)
  </Step>
  <Step title="Grant Contacts">
    Allow Contacts when prompted for attendee name resolution
  </Step>
</Steps>
...
### Cloud transcription (Soniox V5)

When you use the **Cloud** voice processor, Alter routes meeting transcription through the **Soniox V5** engine (beta96+) for higher accuracy on accents and technical vocabulary. Cloud processing requires login and fair-use eligibility; local **Whisper Pro** and **Parakeet** options remain fully offline.
...
## Get value from transcripts

Once a meeting is transcribed, ask Alter for structured follow-up:

<CardGroup cols={2}>
  <Card title="Get action items" icon="check-circle">
    "List all action items from this meeting and who is responsible for each"
  </Card>
  ...
  <Card title="Find specific topics" icon="search">
    "What did we say about the budget, timeline, or API?"
  </Card>
...
<Tip>
  Search across meetings: type `#` in the prompt box to search past conversations, or `>` for meeting transcript history.
</Tip>
...
<CardGroup cols={2}>
  <Card title="Memory" icon="brain" href="/workflows/memory">
    Save and recall useful details from meetings. Use [Memory Catch-Up](/workflows/memory-catch-up) to seed facts from recent transcripts.
  </Card>
  <Card title="Dictation" icon="microphone" href="/workflows/dictation">
    Voice processing options
  </Card>
  <Card title="Settings" icon="settings" href="/how-to/settings-guide#voice">
    Configure meeting recording and speaker matching
  </Card>
