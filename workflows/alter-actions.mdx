...
- **Allowed gateways** — Comma-separated list: `discord`, `telegram`, or `*` for every gateway. Leave empty to keep the action local-only (Hub, QuickHub, etc.).
- The bundled **Ask Anything** action includes **Discord** and **Telegram** by default. Custom actions need the platform name or `*` in **Allowed gateways**.

See [Gateways](/apps-tools/gateways) for setup and chat commands.
