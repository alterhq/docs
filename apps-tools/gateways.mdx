...
| **Start server discussions in threads** (Discord only) | When on, each new top-level server message starts a Discord thread and a separate Alter chat. Messages already inside a thread continue that chat. Direct messages are unchanged. |
...
Actions **opt in** to external platforms. In the **Action Editor**, open **Model → Gateways → Allowed gateways**:

- **`discord`** — available on Discord gateways only
- **`telegram`** — available on Telegram gateways only
- **`*`** — available on every gateway
- **Empty** — local-only (Hub, QuickHub, etc.)

<Tip>
  The bundled **Ask Anything** action is enabled on **Discord** and **Telegram** by default — it is the usual **Default action** for new gateways. Custom actions need `discord`, `telegram`, or `*` under **Allowed gateways**.
</Tip>

People can still switch actions per conversation with `/new [action]` as long as that action is allowed on the platform.
...
| *Action '…' is not enabled for Discord/Telegram* | Open the action in **Action Editor → Model → Gateways** and add the platform or `*`. |
| *No actions are enabled for Discord/Telegram* | Enable at least one action for the platform, and set a valid **Default action** on the gateway. |
...
For broader tool and permission issues, use **Settings → Doctor** or [Tools not working](/common-issues/tool-not-working).

## Related pages

- [Set up a Discord gateway](/apps-tools/gateways-discord)
- [Set up a Telegram gateway](/apps-tools/gateways-telegram)
- [Alter Actions](/workflows/alter-actions) — configure actions and gateway availability
