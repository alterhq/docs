...
<Info>
  **Buzz gateway (internal)** — Internal Alter builds also support a [Buzz gateway](/apps-tools/gateways-buzz) for the dev team. It is not available in public releases yet.
</Info>
...
- **Buzz** *(internal)* — Message a dedicated Nostr identity in an accessible Buzz channel, forum, thread, or direct conversation. See [Buzz gateway (internal)](/apps-tools/gateways-buzz).
...
The Gateways page shows every configured gateway. Use **Add gateway** to create a **Discord** or **Telegram** connection. Internal builds also offer **Buzz**.
...
<CardGroup cols={2}>
  ...
  <Card title="Buzz gateway (internal)" icon="message-square" href="/apps-tools/gateways-buzz">
    Join a Buzz community with an invitation link — dev team only, internal builds.
  </Card>
</CardGroup>
...
| **Start Buzz discussions in threads** (Buzz only, internal) | When on, each triggering top-level shared-channel message starts a Buzz thread and separate Alter chat. **On by default** for new Buzz gateways. |
...
- [Buzz connection fields](/apps-tools/gateways-buzz#step-1--connect-in-alter) *(internal)*
...
| **Buzz** *(internal)* | Buzz gateways only |
...
## Scheduled action output on Gateways

Scheduled Alter Actions can post their automatic opening response to a connected gateway instead of staying Hub-only.

<Steps>
  <Step title="Enable the action on a gateway">
    Open the action in **Action Editor → Gateways** and set **Availability** to the platform, enable the gateway toggles you need, or use **All**.
  </Step>
  <Step title="Turn on scheduled gateway delivery">
    Enable **Start scheduled conversations on a gateway**, choose an enabled **Discord**, **Telegram**, or **Buzz** *(internal)* gateway, then choose a **Destination** (server channel, Telegram chat, or Buzz stream/forum/DM).
  </Step>
  <Step title="Save and test">
    Save the action and run or wait for its schedule. Alter delivers the opening response to the selected destination with any output attachments.
  </Step>
</Steps>

<Tip>
  **Buzz only:** Set **Default scheduled destination** on the gateway when an action should fall back to a destination without naming one each time. See [Buzz gateway (internal)](/apps-tools/gateways-buzz#send-scheduled-action-output-to-buzz).
</Tip>

If a saved destination is no longer accessible, refresh destinations in the Action Editor or gateway settings before the next scheduled run.
