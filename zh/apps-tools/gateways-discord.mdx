...
<Steps>
  ...
  <Step title="Choose 默认 Action and thread behavior">
    **Ask Anything** is the default and works on Discord out of the box. For a custom action, add `discord` or `*` under **Action Editor → Model → Gateways → 允许的 Gateways**.

    Under **Behavior**, optionally turn on **在线程中開始服务器討論** so each new top-level server message gets its own Discord thread and Alter chat. Messages already inside a thread continue that chat. Direct messages are unchanged. If you use this, grant **Create Public Threads** and **Send Messages in Threads** when inviting the bot (**Step 4**).
  </Step>
